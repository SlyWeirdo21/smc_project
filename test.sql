...
WHERE
    ...
    AND payment_history.order_price >= 100000
GROUP BY    
    member.name;
...
SELECT
    ...
    SUM(payment_history.order_price)         AS 구매금액,
    COUNT(payment_history.order_count)       AS 구매개수
FROM
    payment_history,
    product,
    category,
...
WHERE
    ...
    AND payment_history.member_id = member.id
GROUP BY
    product.name,
    category.name,
    member.name
...
SELECT
    SUM(payment_history.order_price)       AS 판매액수,
...
WHERE
        payment_history.member_id = member.id
    AND payment_history.product_id = product.id
    AND to_char(payment_history.order_date, 'YYYYMM') = '202001'
    AND ROWNUM < 2
ORDER BY
    payment_history.order_price
